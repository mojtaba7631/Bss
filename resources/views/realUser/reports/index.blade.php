...
@section('js')
    <script>
        ...
        var doc_finance_modal = $("#doc_finance_modal");

        function getFinanceDoc(phase_id) {
            var url = '/realUser-access/dashboard/reports/finance_docs/get/' + phase_id;
            doc_finance_modal.find('.modal-body').find('.row').remove();
            doc_finance_modal.find('.modal-body').append('<div class="row"></div>');
            $.get(url, function (res) {
                var btn = '';
                var num = 0;
                if (res['documents'].length > 0) {
                    $.each(res['documents'], function (i, val) {
                        num++;
                        btn = '<div class="col-12 col-md-6"><a href="/realUser-access/dashboard/reports/finance_doc/download/' + val['id'] + '" class="btn btn-success mb-4">دانلود سند مالی ' + num + '</botton></div>';
                        doc_finance_modal.find('.modal-body').find('.row').append(btn);
                    });
                } else {
                    btn = '<div class="col-12"><p class="alert alert-warning mb-0">فاقد سند مالی</p></div>';
                    doc_finance_modal.find('.modal-body').find('.row').append(btn);
                }


                doc_finance_modal.modal('show');
            }, 'json');
        }
    </script>
@endsection
@section('content')
        ...
        <div class="container-fluid">
            <div class="row clearfix">
                <div class="col-12">
                    <div class="card">
                        <div class="body">
                            <form method="post" action="{{route('real_reports_search_index')}}">
                                @csrf
                                <div class="row">
                                    ...
                                    @if($searched)
                                        <div class="col-12 col-sm-6 col-md-3 mb-2">
                                            ...
                                            <div class="col-12 col-sm-6 col-md-12 mb-2">
                                                <a type="submit" href="{{route('real_reports_index')}}"
                                        ...
                                        @foreach($reports as $report)
                                            ...
                                            <tr>
                                                ...
                                                @else
                                                    <td>
                                                        @if($report->file_src != '')
                                                            <a href="{{route('real_reports_download_file',['report' => $report->report_id])}}"
                                                ...
                                                @else
                                                    <td>
                                                        @if($report->file_src != '')
                                                            <button onclick="getFinanceDoc({{$report->phases_id}})"
    ...
    <div class="modal" id="doc_finance_modal">
        <div class="modal-dialog">
            <div class="modal-content">
                @csrf
                <!-- Modal Header -->
                <div class="modal-header">
                    <h4 class="modal-title">سندهای مالی</h4>
