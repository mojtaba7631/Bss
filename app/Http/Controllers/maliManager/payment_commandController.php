...
class payment_commandController extends Controller
{
    public function index($payment_id)
    {
        ...
        $payment_info['payment_info_jalali'] = verta($payment_info->date_receipt)->format('Y/m/d');
        ...
        $mali_manager = User::query()
            ->join('user_role', 'user_role.user_id', '=', 'users.id')
            ->where('user_role.roles', 6)
            ->firstOrFail();
        ...
        return view('mali_manager.payment_command.payment_command', compact('project_info', 'phase_info', 'report_info', 'supervisor_info', 'user_name', 'payment_info', 'employer_info', 'main_manager','mali_manager'));
    }
}
