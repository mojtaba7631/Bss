...
class reportsController extends Controller
{
    public function index()
    {
        ...
        return view('legalUser.reports.index', ['reports' => $reports, 'employers' => $employers, 'projects_status' => $projects_status, 'phases_status' => $phases_status, 'searched' => $searched]);
    }
    ...
    public function search_index_legal(Request $request)
    {
        $input = $request->all();
    ...
    public function detail_report(Project $project)
    {
        ...
        return view('legalUser.reports.details_report', ['phases' => $phases, 'projects' => $project]);
    }
    ...
    public function upload_report(Request $request)
    {
        ...
        $validator = Validator::make($input, [
            'file_src' => 'required|mimes:pdf,doc,docx,zip|max:102500',
            'phase' => 'required'
        ]);
        ...
        sms_otp($supervisor_info->mobile, 'checkReport', ['param1' => 'ناظر']);

        alert()->success('گزارش شما برای ناظر ارسال گردید', 'با تشکر')->autoclose(9000);

        return redirect()->route('legal_reports_index');
    }
}
